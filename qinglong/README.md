...
定时类型：crontab
定时规则：2 2 28 * *
白名单：bili_task_.+\.sh|bilitool_lock\.sh
文件后缀：sh
```
...
```
名称：拉取Bili库
命令：ql repo https://github.com/kizunerwe/BiliBiliToolPro.git "bili_task_|bilitool_lock"
定时规则：2 2 28 * *
```
...
充电任务默认关闭。启用前必须配置明确的目标 UP 主 UID；不会自动选择当前登录账号或其他默认目标。任务只会在配置的业务时区月末执行。

在青龙环境变量中添加：

```bash
Ray_ChargeTaskConfig__IsEnable=true
Ray_ChargeTaskConfig__AutoChargeUpId=123456
Ray_ChargeTaskConfig__BusinessTimeZoneId=Asia/Shanghai
```

- `AutoChargeUpId` 必须是目标 UP 主的正整数 UID，未配置或格式错误时任务会直接失败并记录原因。
- `BusinessTimeZoneId` 默认是 `Asia/Shanghai`，用于判断月末；请根据部署环境的实际业务时区填写。
- 不需要充电时保持 `Ray_ChargeTaskConfig__IsEnable=false`，或删除上面的启用变量。
- 启用后仍需满足年度大会员和 B 币券余额等业务条件；不满足时任务会跳过。
...
编辑青龙面板的`配置文件`，新增以下环境变量：

```
export BILI_MODE="bilitool" # bili运行模式，dotnet或bilitool
export BILI_GITHUB_PROXY="" # 可选：可信的下载代理地址，不使用则留空
export BILITOOL_LOCK_WAIT_SECONDS="7200" # 可选：同仓库同分支锁的最大等待秒数
export BILITOOL_UPDATE_CHECK_INTERVAL_SECONDS="86400" # 可选：bilitool发布包检查间隔
```
...
- 同一个仓库同一分支同一时间只允许一个 BiliBiliTool 任务运行，后启动的任务最多等待 `BILITOOL_LOCK_WAIT_SECONDS` 秒；获取锁失败会返回非零。
- `dotnet` 模式只在源码版本变化时重新发布 Console，日常任务直接运行发布产物，不再每次执行 `dotnet run`。
- `bilitool` 模式即使已经安装，也会按 `BILITOOL_UPDATE_CHECK_INTERVAL_SECONDS` 检查发布包；默认最多每 24 小时检查一次。
- 发布包下载、解析、解压或替换失败时保留旧二进制、旧 `tag.txt` 和旧检查时间，任务会返回失败状态。
- 安装或安装后校验失败时，任务会返回失败状态，不再继续运行损坏或缺失的程序。
...
### 5.2. Couldn't find a valid ICU package installed on the system
...
### 5.4. The configured user limit (128) on the number of inotify instances has been reached
