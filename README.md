...
上游停更挺久了，维护者看着也不怎么活跃，而我还有继续用的需求，干脆 fork 出来自立门户，能用就行——反正目前想不出 B 站那土豆服务器还能干啥了。

目前没有复杂的路线图。人走茶凉，唯独我还在这里自娱自乐（？）。还没有收到 Star，也没有其他用户在用的明确反馈，所以维护重心放在我自己的实际需求上，不专门为假想中的大众需求做通用化。

先保证青龙和 Console 稳定运行，遇到明确的问题就修。B 站接口和风控经常变化，与其为了增加功能而增加功能，不如把现有任务做稳、把账号行为做得更自然。有实际需求可以提 Issue。

## 接手后的主要维护

这个 fork 不只包含最近新增的功能。接手后已经围绕实际运行中遇到的问题做过这些调整：

- **设备指纹与风控**：B 站风控开始校验 buvid3 的服务端设备档案，无档案设备调用分享等接口会被 -403 拒绝。程序现在首次运行时自动生成设备指纹并上报建立档案，之后长期稳定复用；设备特征带扰动，另提供 DeviceCookie 配置节手动固定设备值。
...
- **UP 友好模式**：为每日观看和投币前视频增加实际经过时间的播放会话，并提供点赞和独立收藏夹收藏开关，尽量减少只操作互动、不产生账号播放记录的情况。
...
开启后，每日观看任务和每个待投币视频都会先按实际经过时间发送播放心跳，再执行分享、收藏、投币和联动点赞：
